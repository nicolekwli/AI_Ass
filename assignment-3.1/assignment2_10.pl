...
solve_task_bt(Task,Current, ClosedList, D,RR,Cost,NewPos) :-
  ...
  % setof(+Template, +Goal, -Set) 
    % binds Set to the list of all instances of Template satisfying the goal Goal
  setof(c(NextPos, 0), map_adjacent(P, NextPos, empty), PossChildren),
  write('\n setof done'),

  % if this breaks we need to backtrack to the move that led to the deadend
    % use condition
  % TODO: make 99 a larger number
  get_best_next_move(PossChildren, RPath, Obj, ClosedList, 99, Move),
  write('\n next best move done'),
  ...
  ( MoveG == 99 -> write('NEED TO BACKTRACK TO LAST POS'), 
    write($MovePos), 
    RPath_t = [RPath_th | RPath_tt],
    map_distance(RPath_h, RPath_h, H1), 
    write('\nH1'),
    write($H1),
    F1 is G1 + H1, 
    write('F1'),
    ...
    ;map_distance(MovePos, MovePos, H1), F1 is G1 + H1, NextMovePos = MovePos, RPathNew = RPath, ClosedListNew = ClosedList
  ),
